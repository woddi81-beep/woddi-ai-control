...
Webbasierte MCP-Zentrale fuer `woddi-ai`, lokale Satelliten und entfernte MCP-Instanzen.
...
Damit bleiben lokale Anpassungen bei `git pull` erhalten.
...
- MCP-Manager fuer `docs`, `files`, `netbox` und `remote_http`
- Remote-MCP-Handshake
- Pflege von `passwd.json` und `personas/*.md`
- Runtime-, LLM- und NetBox-Verwaltung
...
- `mcps.local.json`
  lokale und entfernte MCP-Definitionen
- `personas/default.md`
  Standardverhalten
- `personas/network-ops.md`
  Beispiel fuer eine NetBox-/Ops-Persona

Beim ersten Start werden diese Dateien aus `config/` erzeugt, falls sie noch fehlen.
